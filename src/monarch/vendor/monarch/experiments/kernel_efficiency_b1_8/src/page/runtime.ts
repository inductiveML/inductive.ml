import {quantLayoutKernel} from './quant_layout.ts';
import {onlineFlash,subgroupMerge} from './online_flash.ts';
import {efficientMlp,efficientFold,swizzleStage,privateMatvec,preNormalizedMatvec,inputNormKernel,convProjectionKernel,maskedDot,magicDot} from './efficiency.ts';
import {candidateOf} from "./plans.ts";
import {setKeyPadding,setFlashMode,setMlpMode} from "./fused_wgsl.ts";
...
import {matvecKernel} from './matvec_wgsl.ts';
import { planStep, type FusedWeight, type PlannedDispatch, type PlanOptions } from "./efficiency_plan.ts";
import {efficiencyPlan,hasNormOnce} from "./efficiency_plan.ts";
import { maxSlicesOf, planSlicesFor } from "../../../flash_core_split_n_b1_4/src/pure/plan_options.ts";
...
import {
  ...
  type RuntimeShape,
  type Workgroups,
} from "../../../flash_core_split_n_b1_4/src/page/wgsl.ts";
...
export interface FusedRuntime {
  ...
  encodeChunk(encoder: GPUCommandEncoder, tokens: number, timestamps: GPUComputePassTimestampWrites | null): void;
...
export function createRuntime(
  ...
  const normalized = make(residualBytes, "normalized");
  ...
  const gateSoAStride=2*shape.ffn*shape.hidden/2,downSoAStride=shape.ffn*shape.hidden/2;
  const repackQuant=(kind:'gate'|'down'):GPUBuffer|null=>{
    if(!armPlans.some(a=>{const mode=candidateOf(a.key).mlpSoA;return mode===kind||mode==='both';}))return null;
    const stride=kind==='gate'?gateSoAStride:downSoAStride;
    const output=make(stride*shape.layerTypes.length,'mlp_'+kind+'_soa');
    const kernel=quantLayoutKernel(shape.hidden,shape.ffn,kind),entry=pipelineFor(device,kernel,'repack_'+kind);
    const enc=device.createCommandEncoder(),pass=enc.beginComputePass();pass.setPipeline(entry.pipeline);
    for(let layer=0;layer<shape.layerTypes.length;layer++){
      const input=section(`L${layer}.mlp.${kind==='gate'?'gate_up.quant':'down.kquant'}`);
      if(input.bytes!==stride)throw new Error('SoA source size mismatch');
      const group=device.createBindGroup({layout:entry.layout,entries:[{binding:0,resource:bufferBinding(weights,input)},{binding:1,resource:cacheSlice(output,layer,stride)}]});
      pass.setBindGroup(0,group);pass.dispatchWorkgroups(Math.ceil(stride/4/256));
    }
    pass.end();device.queue.submit([enc.finish()]);return output;
  };
  const gateSoA=repackQuant('gate'),downSoA=repackQuant('down');
  ...
  const buildArm = (armPlan: ArmPlan): Arm => {
    const tuning=candidateOf(armPlan.key);
    setKeyPadding(tuning.padding);setFlashMode(tuning.flashMode);setMlpMode(tuning.mlpMode??'carried');
    const plan = efficiencyPlan(planStep(shape, armPlan.options),tuning,shape,armPlan.options.position,armPlan.options.queries);
    ...
    const matvecStep = (
      ...
      const variant=tuning.matvec[dispatch.site]?.variant??"carried";
      const convFused=dispatch.kind==='conv_proj_core';
      const normOnce=hasNormOnce(tuning,dispatch);
      const key = `${mode}_${rows}x${cols}${zeroPoints ? "_zp" : ""}_${occupancy}_${variant}_n${tuning.normSubgroup??false}_sw${tuning.matvecStageSwizzle??false}_pv${tuning.matvecPrivate??false}_once${normOnce}_conv${convFused}_mask${tuning.maskMatvec??false}_magic${tuning.magicMatvec??false}`;
      const entry = cache.get(key, () =>
        magicDot(maskedDot(convProjectionKernel(preNormalizedMatvec(privateMatvec(swizzleStage(matvecKernel([mode, rows, cols, zeroPoints, matvecGeometry, runtimeShape, preScale, variant],tuning.normSubgroup??false),tuning.matvecStageSwizzle??false),mode,preScale,tuning.matvecPrivate??false),normOnce),convFused),tuning.maskMatvec??false),tuning.magicMatvec??false),
      );
      ...
      if(normOnce)full.residual=whole(normalized);
      if (zeroPoints) {
    ...
    for (const dispatch of plan) {
      ...
      switch (dispatch.kind) {
        case "input_norm": {
          const target=dispatch.name.replace(/\.input_norm$/,'');
          const gamma=target.endsWith('.mlp.fused')?target.replace(/\.fused$/,'.gate_up.norm'):target+'.norm';
          const preScale=target==='head'?options.headPreScale:1;
          const entry=cache.get('input_norm_'+dispatch.workgroupSize+'_'+preScale,()=>inputNormKernel(dispatch.workgroupSize,preScale));
          record(dispatch,entry,{residual:residualOf(f.residualIn),gamma:bufferBinding(weights,section(gamma)),projection:whole(normalized)},()=>[1,1,1]);
          break;
        }
        case "embed": {
        ...
        case "conv_proj_core": {
          ...
          matvecStep(dispatch,"norm_matvec",weight.rows,weight.cols,weight.zeroPoints,1,dispatch.name,{
            gamma:bufferBinding(weights,section(dispatch.name+'.norm')),residual:residualOf(f.residualIn),destination:whole(mid),
            blob:bufferBinding(weights,section(dispatch.name.replace(/\.in_proj$/,'.core.taps'))),cache:cacheSlice(convCache,convIndex,convLayerBytes),
          });convIndex++;break;
        }
        case "norm_matvec": {
          const weight = weightOf(dispatch);
        ...
        case "attn_core_flash": {
          const layer = dispatch.layer;
          const entry = cache.get(`attn_core_flash_L${layer}_s${splits}_t${flashReach}_p${tuning.padding}_${tuning.flashMode}_online${tuning.flashOnline??false}`, () =>
            tuning.flashOnline?onlineFlash(runtimeShape,attnCoreOffsets(layer),splits):attnCoreFlashKernel(runtimeShape, attnCoreOffsets(layer), options.flashCoreWorkgroup, splits, flashReach),
          );
          record(
            dispatch,
            entry,
        ...
        case "attn_merge": {
          const entry = cache.get(`attn_merge_s${splits}_sg${tuning.mergeSubgroup??false}`, () =>
            subgroupMerge(attnMergeKernel(runtimeShape, options.mergeWorkgroup, splits),splits,tuning.mergeSubgroup??false),
          );
          record(
            dispatch,
            entry,
        ...
        case "mlp_fused": {
          ...
          blockStep(
            dispatch,
            `mlp_fused_${tuning.key}_L${layer}_f${f.foldSlices}_${tuning.mlpMode??"carried"}_s${tuning.mlpSlices??80}_w${tuning.mlpWorkgroup??256}`,
            {...options.mlpFused,slices:tuning.mlpSlices??options.mlpFused.slices,workgroupSize:tuning.mlpWorkgroup??options.mlpFused.workgroupSize},
            (geometry) => efficientMlp(mlpFusedKernel(runtimeShape, mlpOffsets(layer), geometry),tuning),
            {
              ...((tuning.mlpSoA==='gate'||tuning.mlpSoA==='both')&&gateSoA?{weights:cacheSlice(gateSoA,layer,gateSoAStride)}:{}),
              ...((tuning.mlpSoA==='down'||tuning.mlpSoA==='both')&&downSoA?{source:cacheSlice(downSoA,layer,downSoAStride)}:{}),
            },
          );
          break;
        }
        case "fold": {
          ...
          const entry = cache.get(`fold_${tuning.key}_${f.foldSlices}`, () =>
            efficientFold(foldKernel(runtimeShape, options.fold.workgroupSize, tuning.foldLanes??options.fold.threadsPerRow, f.foldSlices),tuning),
          );
          record(
            dispatch,
            entry,
            { residual_out: residualOf(f.residualOut), partials_in: deferredOf(f.partialsIn) },
            () => foldWorkgroups(runtimeShape, options.fold.workgroupSize, tuning.foldLanes??options.fold.threadsPerRow),
          );
          break;
        }
        case "norm_head": {
  ...
  const owned = [
    ...(gateSoA?[gateSoA]:[]),...(downSoA?[downSoA]:[]),
    normalized,
    residual,
    residualB,
    deferredA,
  ...
  return {
    ...
    get tokensPerPass():number{return candidateOf(active.key).tokensPerPass??1;},
    ...
    encodeChunk(encoder:GPUCommandEncoder,count:number,timestamps:GPUComputePassTimestampWrites|null):void{
      const pass=encoder.beginComputePass(timestamps?{timestampWrites:timestamps}:{});
      for(let token=0;token<count;token++)for(const step of stepsFor(1)){
        pass.setPipeline(step.pipeline);pass.setBindGroup(0,step.bindGroup);
        const [x,y,z]=step.workgroups(1);pass.dispatchWorkgroups(x,y,z);
      }
      pass.end();
    },
    encodeToken(encoder: GPUCommandEncoder, queries: number, timestamps: GPUComputePassTimestampWrites | null): void {
